...
export function buildShareText(p: Person, txns: Txn[], lang: Lang): string {
  const t = tFor(lang);
  const tp = tpFor(lang);
  ...
  const balLine =
    b > 0
      ...
      : b < 0
        ...
        : `*${t('shareBalZero')}*`;
  return `📒 *${tp('shareTitle', { n: p.name })}*\n\n${hist.join('\n')}\n\n${balLine}`;
}

export async function shareOnWhatsApp(p: Person, txns: Txn[], lang: Lang): Promise<void> {
  const text = buildShareText(p, txns, lang);
