...
/* amount cell coloured by direction (green = receivable/out, red = in) */
const moneyCell = (cell: { amount: number; dir: 'in' | 'out'; date?: string | null } | null, lang: string): string => {
  if (!cell) return '<td class="m"></td>';
  const cls = cell.dir === 'out' ? 'g' : 'r';
  const d = cell.date ? `<span class="dt">${dstr(cell.date, lang)}</span>` : '';
  return `<td class="m ${cls}">${fmt(cell.amount)}${d}</td>`;
};
...
<table><thead><tr>
  <th>${t('bookSno')}<\/th><th class="name">${t('fName')}<\/th><th>${t('bookOpening')}<\/th>
  <th>1<\/th><th>2<\/th><th>3<\/th><th>4<\/th><th>5<\/th><th>${t('bookBalance')}<\/th>
<\/tr><\/thead><tbody>`);

  rows.forEach((r, i) => {
    const ref = r.person.ref ? `<span class="ref">${esc(r.person.ref)}<\/span>` : '';
    const opening = r.opening ? moneyCell(r.opening, lang) : '<td class="m"></td>';
    const cells = Array.from({ length: 5 }, (_, k) => moneyCell(r.entries[k] ?? null, lang)).join('');
    const bal =
      r.balance === 0
        ? `<td class="bal settled">${t('settled')}<\/td>`
        : `<td class="bal ${r.balance > 0 ? 'g' : 'r'}">${fmt(r.balance)}<\/td>`;
    parts.push(
      `<tr><td class="sno">${i + 1}<\/td><td class="name">${esc(r.person.name)}${ref}<\/td>${opening}${cells}${bal}<\/tr>`
    );
  });

  parts.push(`<\/tbody><tfoot><tr>
  <td colspan="8" style="text-align:right">${esc(tp('totalsLine', { n: tot.count, r: fmt(tot.recv), g: fmt(tot.give) }))}<\/td>
