...
```jsonc
"version": "1.4.0",        // user-facing version → "v1.4.0"
"android": { "versionCode": 5 }   // integer build number → "(5)"; MUST increase each release
```

Before a release build:

1. Bump `expo.version` (e.g. `1.3.0` → `1.4.0`) and increment
