...
import { dayCountLabel, relativeInvLabel, today } from '../lib';
...
export function InvitationChip({ date }: { date: string }) {
  ...
  if (rel.kind === 'overdue')
    return <StatusChip kind="neg" label={dayCountLabel(rel.d, 'daysAgo1', 'daysAgo', t, tp)} />;
  ...
  return <StatusChip kind="gold" label={dayCountLabel(rel.d, 'daysLeft1', 'daysLeft', t, tp)} />;
}
